...
namespace Testing
{
	...
	[Test]
	public async Task CustomValidation_GivenDefaultMethodExists_GeneratesAsyncPath(
		CancellationToken cancellationToken
	)
	{
		var source =
...
namespace Testing
{
	...
	public class WithDefault
	{
		...
		await Assert
			.That(generated)
			.Contains("await global::Testing.WithDefault.CustomValidationAsync");
	...
	[Test]
	public async Task CustomValidation_GivenOverriddenMethodName_GeneratesAsyncPath(
		CancellationToken cancellationToken
	)
	{
		var source =
...
namespace Testing
{
	...
	public class WithOverride
	{
		...
		await Assert
			.That(generated)
			.Contains("await global::Testing.WithOverride.ValidateRulesAsync");
	}

	[Test]
	public async Task CustomValidation_GivenExplicitNameMissing_ProducesZODSGEN007(
		CancellationToken cancellationToken
	)
	{
		const string source =
...
namespace Testing
{
	...
	public class MissingMethod { public string? Name { get; set; } }
		...
		await AssertNoGeneratorExceptions(driverResult);
		var diags = GetGeneratorDiagnostics(driverResult);
		await Assert.That(diags.Any(static d => d.Id == "ZODSGEN007")).IsTrue();
	}

	[Test]
	public async Task CustomValidation_GivenInvalidReturnType_ProducesZODSGEN008(
		CancellationToken cancellationToken
	)
	{
		var source =
...
namespace Testing
{
	...
	public class BadReturn
	{
		...
		var driverResult = await GenerateAsync(source, cancellationToken);
		var diags = GetGeneratorDiagnostics(driverResult);
		await Assert.That(diags.Any(static d => d.Id == "ZODSGEN008")).IsTrue();
	}

	[Test]
	public async Task CustomValidation_GivenWrongParameterCount_ProducesZODSGEN009(
		CancellationToken cancellationToken
	)
	{
		var source =
...
namespace Testing
{
	...
	public class WrongParams
	{
		...
		var driverResult = await GenerateAsync(source, cancellationToken);
		var diags = GetGeneratorDiagnostics(driverResult);
		await Assert.That(diags.Any(static d => d.Id == "ZODSGEN009")).IsTrue();
	}
...
namespace Testing
{
	...
	public class WrongModel
	{
		...
		var driverResult = await GenerateAsync(source, cancellationToken);
		var diags = GetGeneratorDiagnostics(driverResult);
		await Assert.That(diags.Any(static d => d.Id == "ZODSGEN010")).IsTrue();
	}
...
namespace Testing
{
	...
	public class WrongCT
	{
		...
		var driverResult = await GenerateAsync(source, cancellationToken);
		var diags = GetGeneratorDiagnostics(driverResult);
		await Assert.That(diags.Any(static d => d.Id == "ZODSGEN011")).IsTrue();
	}

	[Test]
	public async Task CustomValidation_GivenGenericMethod_ProducesZODSGEN012(
		CancellationToken cancellationToken
	)
	{
		var source =
...
namespace Testing
{
	...
	public class GenericMethod
	{
		...
		var driverResult = await GenerateAsync(source, cancellationToken);
		var diags = GetGeneratorDiagnostics(driverResult);
		await Assert.That(diags.Any(static d => d.Id == "ZODSGEN012")).IsTrue();
	}

	[Test]
	public async Task CustomValidation_GivenInstanceMethod_ProducesZODSGEN013(
		CancellationToken cancellationToken
	)
	{
		var source =
...
namespace Testing
{
	...
	public class InstanceMethod
	{
		...
		var driverResult = await GenerateAsync(source, cancellationToken);
		var diags = GetGeneratorDiagnostics(driverResult);
		await Assert.That(diags.Any(static d => d.Id == "ZODSGEN013")).IsTrue();
	}

	[Test]
	public async Task CustomValidation_GivenPrivateMethod_ProducesZODSGEN014(
		CancellationToken cancellationToken
	)
	{
		var source =
...
namespace Testing
{
	...
	public class PrivateMethod
	{
		...
		var driverResult = await GenerateAsync(source, cancellationToken);
		var diags = GetGeneratorDiagnostics(driverResult);
		await Assert.That(diags.Any(static d => d.Id == "ZODSGEN014")).IsTrue();
	}
...
namespace Testing
{
	...
	[Test]
	public async Task CustomValidation_GivenInvalidMethodName_ProducesZODSGEN016(
		CancellationToken cancellationToken
	)
	{
		const string source =
...
namespace Testing
{
	...
	public class BadName { public string? Name { get; set; } }
		...
		var driverResult = await GenerateAsync(source, cancellationToken);
		var diags = GetGeneratorDiagnostics(driverResult);
		await Assert.That(diags.Any(static d => d.Id == "ZODSGEN016")).IsTrue();
	}

	[Test]
	public async Task CustomValidation_GivenRefParameter_ProducesZODSGEN019(
		CancellationToken cancellationToken
	)
	{
		var source =
...
namespace Testing
{
	...
	public class RefParam
	{
		...
		var driverResult = await GenerateAsync(source, cancellationToken);
		var diags = GetGeneratorDiagnostics(driverResult);
		await Assert.That(diags.Any(static d => d.Id == "ZODSGEN019")).IsTrue();
	}

	[Test]
	public async Task CustomValidation_GivenOverloadsButOnlyOneValid_SelectsValid(
		CancellationToken cancellationToken
	)
	{
		var source =
...
namespace Testing
{
	...
	public class OverloadsOneValid
	{
		...
		var driverResult = await GenerateAsync(source, cancellationToken);
		var diags = GetGeneratorDiagnostics(driverResult);
		await Assert.That(diags.Any(static d => d.Id is "ZODSGEN008" or "ZODSGEN009")).IsTrue();
		await Assert.That(diags.Any(static d => d.Id == "ZODSGEN015")).IsFalse();
	}
}
