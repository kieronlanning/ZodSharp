...
#pragma warning restore IDE0060 // Remove unused parameter
	...
	{
		var schema = JsonSerializer.Deserialize<JsonSchemaDefinition>(
			jsonSchema,
	...
	static IZodSchema<object, object> CreateLiteralSchema(object? value)
	{
		...
		{
			return CreateLiteralFromJsonElement(element);
		}
	...
	static IZodSchema<object, object> CreateLiteralFromJsonElement(JsonElement element)
	{
		return element.ValueKind switch
		{
			JsonValueKind.String => new LiteralSchemaWrapper<string>(element.GetString()!),
			JsonValueKind.Number => element.TryGetInt64(out var longValue)
				? new LiteralSchemaWrapper<long>(longValue)
				: new LiteralSchemaWrapper<double>(element.GetDouble()),
			JsonValueKind.True => new LiteralSchemaWrapper<bool>(true),
			JsonValueKind.False => new LiteralSchemaWrapper<bool>(false),
			JsonValueKind.Null => new NullSchemaWrapper(),
			_ => new AnySchema(),
		};
	}
	...
	class NumberSchemaWrapper(ZodNumber inner) : IZodSchema<object, object>
	{
		public ValidationResult<object> Validate(object value)
		{
			...
			if (value is double d)
				...
				num = f;
			else if (value is JsonElement element && element.ValueKind == JsonValueKind.Number)
			{
				num = element.GetDouble();
			}
			else
			{
				return ValidationResult<object>.Failure(
	...
	class NullSchemaWrapper : IZodSchema<object, object>
	{
		public ValidationResult<object> Validate(object value)
		{
			if (
				value != null
				&& !(value is JsonElement element && element.ValueKind == JsonValueKind.Null)
			)
			{
				return ValidationResult<object>.Failure(
					new ValidationError(
		...
		: IZodSchema<object, object>
	{
		public ValidationResult<object> Validate(object value)
		{
			...
			{
				items = [.. element.EnumerateArray().Cast<object?>()];
			}
...
#pragma warning disable IDE0072 // Add missing cases
				object? extracted = element.ValueKind switch
				{
					JsonValueKind.String => element.GetString(),
