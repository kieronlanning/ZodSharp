...
partial class ZodSchemaGenerator
{
	...
	)
	{
		...
		writer
			...
			.Using(TypeLibrary.ZodSharpCoreNamespace)
			.NewLine();
			...
			method =>
			{
				if (schema.TargetCanBeNull)
				{
					method.IfBlock(
						...
						ifBody =>
							ifBody.Return(
								$"{TypeLibrary.ValidationResult.MakeGeneric(schema.TargetType)}.Failure({TypeLibrary.ValidationError}.Create({"invalid_type".Surround()}, {"Value cannot be null".Surround()}, EmptyPath))"
							)
					);
				}
				...
				method.IfBlock(
					...
					ifBody =>
						ifBody.Return(
							TypeLibrary.ValidationResult.MakeGeneric(schema.TargetType).StaticMember("Failure(errors)")
						)
				);

				method
					.Line()
					.Return(TypeLibrary.ValidationResult.MakeGeneric(schema.TargetType).StaticMember("Success(value)"));
			}
		);
	...
	)
	{
		...
		var funcType = PurviewTypeLibrary
			.System.Func.MakeGeneric(schema.TargetType, PurviewTypeLibrary.System.Boolean)
			.AsTypeReference();
		...
		void WriteFailure(CodeWriter body, string fallbackMessage) =>
			body.Return(
				$"{validationResultType}.Failure({TypeLibrary.ValidationError}.Create({"validation_failed".Surround()}, message ?? {fallbackMessage.Surround()}, EmptyPath))"
			);
				...
				body.Foreach(
					...
					foreachBody =>
						foreachBody.MethodCall(
							"AddError",
							"ref errors",
							$"{TypeLibrary.ValidationError}.Create(error.Code, error.Message, {immutableArrayType}.Create(prefix).AddRange(error.Path), error.Parameters, error.Origin, error.Minimum, error.Maximum, error.Inclusive)"
						)
				)
		);
				...
				body.Foreach(
					...
					foreachBody =>
						foreachBody.MethodCall(
							"AddError",
							"ref errors",
							$"{TypeLibrary.ValidationError}.Create(error.Code, error.Message, {immutableArrayType}.Create(prefix, index.ToString()).AddRange(error.Path), error.Parameters, error.Origin, error.Minimum, error.Maximum, error.Inclusive)"
						)
				)
		);
