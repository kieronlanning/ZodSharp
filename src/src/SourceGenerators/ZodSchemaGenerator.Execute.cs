...
partial class ZodSchemaGenerator
{
		...
		try
		{
			using (generationContext.Writer.Begin())
			{
				...
				var registrationSource =
					"// <auto-generated />\n"
					+ "using ZodSharp.Core;\n\n"
					+ $"[assembly: global::ZodSharp.Core.ZodSchemaGenerated(typeof({fullTypeName}))]\n";
				context.AddSource(
	...
	)
	{
		...
		var customValidation = ResolveCustomValidationMethod(classSymbol, zodSchemaAttribute, generationContext);
		if (customValidation.Diagnostics.Length > 0)
			diagnostics.AddRange(customValidation.Diagnostics);

		GenerateValidatorAdapter(generationContext, classSymbol, schemaName, fullTypeName, modifier, customValidation);
	...
	)
	{
		...
		)
		{
			...
			generationContext.Writer.WriteLine();
			GenerateValidateAsyncMethod(generationContext, schemaName, fullTypeName, customValidation);
		}
	}
