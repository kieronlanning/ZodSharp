...
- Use a separate test project with naming convention `[ProjectName].{TestType}Tests`
  - `TestType` is the type of test being created, e.g. `Unit`, `Integration`, `Performance`, etc.
- Reference TUnit package, the namespace is automatically included and should not be added to the files.
- Create test classes that match the classes being tested (e.g., `CalculatorTests` for `Calculator`), always match the test class structure (.e.g., `Services/CalculatorTests.cs` for `Services/Calculator.cs`)
- Use .NET SDK test commands: `dotnet test` for running tests
- TUnit requires .NET 8.0 or higher

## Running Tests

- Use `dotnet test` in the solution file's folder.
- Filtering tests is achieved through the `--treenode-filter` argument, passing in `"/*/*/*/*/"`
